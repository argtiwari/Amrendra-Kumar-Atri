...
export default function Sampark() {
  ...
  const handleSubmit = (e) => {
    ...
    emailjs.sendForm(
      "service_mifijro",   // 🔹 apna Service ID
      "template_eooeyzf",  // 🔹 apna Template ID
      form.current,
      "dfBzPJ2CdxeO6gbwr"      // 🔹 apna Public Key
    )
      .then(
        (result) => {
        ...
        (error) => {
          ...
          alert("❌ Sandesh bhejne me dikkat hui, firse try karein.");
        }
      );
  };
        ...
        <div data-aos="fade-right" className="text-center md:text-left">
          ...
          <div className="mb-4">
            <div className="inline-block bg-white/90 rounded-lg px-4 py-2 shadow text-sm text-gray-700 font-medium">
              <span className="font-semibold text-orange-700">पता:</span> रमा मेंशन, बाय-पास रोड कचेहरी-जमुई, 811307  बिहार Mob-7004009363
            </div>
          </div>
          {/* Live Counter / Ticker */}
        ...
        <div data-aos="fade-left" className="bg-white/95 p-8 rounded-2xl shadow-xl">
          ...
          {!submitted ? (
            <form ref={form} onSubmit={handleSubmit} className="space-y-4">
              <input
                type="text"
                name="user_name"   // 🔹 yeh name attribute EmailJS ke template ke liye important hai
                placeholder="Naam"
              ...
              <input
                type="text"
                name="user_email"  // 🔹 yeh bhi zaroori hai
                placeholder="Mobile / Email"
                className="w-full p-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500"
                required
              />
              <textarea
                name="message"    // 🔹 yeh bhi EmailJS template me use hoga
